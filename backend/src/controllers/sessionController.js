...
export async function getActiveSessions(_,res) {
    try{
        ...
        .sort({createdAt:-1}) //desc order as -1
        .limit(20);
...
export async function joinSession(req,res) {
    try{
        ...
        //check if session is already full -has a participant already
...
export async function endSession(req,res) {
    try{
        ...
        if(session.participant) return res.status(404).json({message:"Session not found"});
        
        //endsession only if user is the host 
